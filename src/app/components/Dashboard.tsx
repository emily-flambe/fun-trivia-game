			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 stagger">
				{categories.map((cat) => (
					<a
						...
						className="group bg-surface-raised rounded-2xl p-6 hover:bg-surface-hover transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-black/20 relative overflow-hidden"
					>
						<div className="absolute top-0 left-0 right-0 h-1 rounded-t-2xl" style={{ backgroundColor: cat.color }} />
						<h3 className="text-lg font-semibold mb-1 group-hover:text-white transition-colors">{cat.name}</h3>
